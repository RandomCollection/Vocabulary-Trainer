...
class VocabularyTrainer(MDApp):
	...
	def solve(self):
		try:
			...
			self.root.ids.label_word_out.text = "Please click the [i]START[/i] button to start"

	def check(self):
		android.show_keyboard()
		if self.words_out is None:  # first press on START when application is started
			self.root.ids.please_translate.text = "Please translate"
			self.next()
			self.root.ids.label_button_check.text = "CHECK"
		else:
			if self.root.in_class.text == "":
				self.root.ids.label_word_out.text = "Please enter a translation into the text field"
			elif self.root.in_class.text == self.words_out[self.z]:
				self.root.ids.label_word_out.text = "[color=238823]Correct =)[/color]"
				Clock.schedule_once(lambda _: self.sleeper_correct(), 1)
	...
	def import_db(self):
		try:
			r = requests.get(url=r"http://raw.github.com/RandomCollection/Vocabulary-Trainer/main/data.db", timeout=5)
			open("data.db", "wb").write(r.content)
			self.root.ids.label_update_status.text = "Import successful!"
			self.update_general()
		except Exception as e:
			self.root.ids.label_update_status.text = f"Import not successful! Error:\n'{str(e)}'"
	...
	def export_db(self):
		try:
			...
			self.root.ids.label_update_status.text = f"Export not successful! Error:\n'{str(e)}'"
	...
	def update_sensitivity(self) -> MDDropdownMenu:
		return MDDropdownMenu(
			...
			items=[
				...
				} for sensitivity in [1, 2, 50, 250, 500]
			],
			width_mult=3,
			max_height=dp(224),
			background_color="#A7FFEB",
