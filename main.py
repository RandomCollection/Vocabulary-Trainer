...
def create_label_home() -> str:
	...
	days = (datetime.date.today() - datetime.datetime.strptime(last_date, '%Y-%m-%d').date()).days
	if days == 0:
...
class VocabularyTrainer(MDApp):
	...
	label_counter_streak_text = StringProperty(defaultvalue="")
	label_counter_streak_number = StringProperty(defaultvalue="")
	label_counter_words_text = StringProperty(defaultvalue="")
	label_counter_words_number = StringProperty(defaultvalue="")
	...
	def __init__(self, **kwargs):
		...
		self.init_label_counters_text()
		self.init_label_counters_number()
		self.init_label_statistics()
		self.table_statistics = create_table_statistics()
		# Datatables
		self.columns_data = [("Word", dp(20)), ("Translation", dp(40)), ("Level", dp(10))]
	...
	def reset_statistics(self):
		self.distinct_category = create_str_from_list(char=db.get_distinct_categories(level=self.distinct_level))
		self.distinct_level = create_str_from_list(char=db.get_distinct_levels(category=self.distinct_category))
	...
	def on_leave_settings(self):
		self.root.ids.label_reset_status.text = ""
	...
	def export_db(self):
		try:
			...
			self.root.ids.label_update_status.text = cfg.LABEL_EXPORT_0.format(e=str(e))

	def on_leave_update(self):
		self.root.ids.label_update_status.text = ""
	...
	def init_label_counters_number(self):
		self.label_counter_streak_number = choose_color(
			indicator=self.setting_counter_on,
			text=cfg.LABEL_COUNTER_STREAK_NUMBER.format(
				current=f"{self.statistic_streak_current:,}",
				best=f"{self.statistic_streak_best:,}"
			)
		)
		self.label_counter_words_number = choose_color(
			indicator=self.setting_counter_on,
			text=cfg.LABEL_COUNTER_WORDS_NUMBER.format(
				current=f"{self.statistic_words_current:,}",
				total=f"{self.statistic_words_total:,}"
			)
		)

	def init_label_counters_text(self):
		self.label_counter_streak_text = choose_color(
			indicator=self.setting_counter_on, text=cfg.LABEL_COUNTER_STREAK_TEXT
		)
		self.label_counter_words_text = choose_color(
			indicator=self.setting_counter_on, text=cfg.LABEL_COUNTER_WORDS_TEXT
		)

	def init_label_statistics(self):
		self.statistic_number_of_words = db.get_number_of_words(language='ES')
		self.statistic_number_of_categories = len(db.get_distinct_categories_all())
	...
	def update_statistics(self):
		...
		self.init_label_counters_number()
		db.update_counters(streak_best=self.statistic_streak_best, words_total=self.statistic_words_total)
		db.update_constants_date(value=datetime.date.today())
